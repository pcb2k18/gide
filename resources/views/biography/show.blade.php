        ...
        @endif
                        ...
                        <div class="space-y-12">
                            ...
                            @if(!empty($data['quickFacts']['facts']))
                                <section id="quick-facts-mobile" class="lg:hidden">
                            ...
                            <div class="prose prose-lg max-w-none">
                                @php
                                    $sectionKeys = ['trendingFocus', 'earlyLife', 'career', 'personalLife', 'death', 'netWorth'];
                                @endphp

                                @foreach($sectionKeys as $sectionKey)
                                    @if(!empty($data[$sectionKey]) && !empty($data[$sectionKey]['title']) && ($sectionKey !== 'death' || !empty($data[$sectionKey]['content'])))
                                        @php $sectionData = $data[$sectionKey]; @endphp
                                        <section id="{{ $sectionData['anchorId'] ?? Str::slug($sectionKey) }}">
                                            <div class="bg-white rounded-2xl @if($loop->first) bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 @endif p-8 sm:p-10">
                                                ...
                                                @if($sectionKey === 'career' && !empty($sectionData['timeline']))
                                                    <div class="my-8 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-6 not-prose">
                                                        <h3 class="text-lg font-semibold text-gray-800 mb-4 flex items-center"><i data-lucide="clock" class="w-5 h-5 mr-2 text-blue-600"></i>Career Timeline</h3>
                                                        <div class="space-y-4">
                                                            @foreach($sectionData['timeline'] as $item)
                                                                <div class="flex items-start space-x-4 p-3 bg-white rounded-lg shadow-sm">
                                                                    <div class="flex-shrink-0 w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center font-bold text-blue-600">{{ $item['year'] ?? '' }}</div>
                                                                    <div>
                                            ...
                                            <div class="space-y-4" x-data="{ openFaq: null }">
                                                @foreach($data['faqs']['questions'] as $index => $faq)
                                                    <div class="border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                                                        ...
                                                        <div x-show="openFaq === {{ $index }}" x-transition class="px-6 pb-5 text-gray-700 bg-blue-50/50"><div class="text-base leading-relaxed">{!! $faq['answer'] !!}</div></div>
                                                    </div>
                                                @endforeach
                                            </div>
                                        </div>
                                                        ...
                                                        <li class="flex bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition-colors">
                                                            ...
                                                            <div class="flex-1">
                                                                ...
                                                                @if(!empty($source['publisher']))
                                                                    <p class="text-slate-600 mt-1 text-sm">Publisher: {{ $source['publisher'] }}</p>
                                                                @endif
                                                            </div>
                                                        </li>
                                                    @endif
